...
func attachmentRoutes(router fiber.Router, db *mongo.Database) {
	router.Post("/:uploadID.:fileExt", func(c *fiber.Ctx) error {
		// Get upload ID
		...
		if !tokenValid || tokenClaims.Type != "upload_file" {
			return c.SendStatus(fiber.StatusUnauthorized)
		}
		if tokenClaims.Upload.UploadID != uploadID || tokenClaims.Upload.UploadType != "avatar" {
			return c.SendStatus(fiber.StatusUnauthorized)
		}
		...
		// Convert file contents into a byte slice
		...
		if err != nil {
			if err == mongo.ErrNoDocuments {
				...
				// Upload to SeaweedFS
				fid, err := saveFile(image, uploadID+".webp")
				if err != nil {
					log.Fatalln(err)
				}
		...
		newUpload := Upload{
			...
			Fid:       upload.Fid,
			Type:      "avatar",
			Filename:  uploadID + ".webp", // we use the upload ID here instead of the original name for privacy
			Mime:      "image/webp",
			Size:      upload.Size,
